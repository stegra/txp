...
SF;
		return parent::html_head($event).n.script_js($js).n;
	}
	...
	function header()
	{
		...
		foreach ($this->menu as $tab)
		{
			...
			$out[] = "<li class='primary {$class}'><a href='?event={$tab['event']}&win={$WIN['winid']}'>{$tab['label']}</a>";
			if (!empty($tab['items']))
			{
				$out[] = '<ul>';
				foreach ($tab['items'] as $item)
				{
					...
					$out[] = "<li class='secondary {$class}'><a href='?event={$item['event']}&win={$WIN['winid']}'>{$item['label']}</a></li>";
				}
				$out[] = '</ul>';
	...
	function footer()
	{
		return '<div id="end_page">'.n.
			'<a href="http://textpattern.com/" id="mothership"><img src="theme/classic/txp_img/carver.gif" width="60" height="48" border="0" alt="" /></a>'.n.

...
// -------------------------------------------------------------
// change: took out "Posted <=now()" from search query
	...
	{	
		...
		$rs = safe_rows(
			...
			and Status in (4,5) order by score desc limit 40");
...
// -------------------------------------------------------------
// change: search result excerpt between <div> tag, not <br>
	
	function legacy_form() 
	{	// lose this eventually
		return '<h3><txp:search_result_title /></h3>
		<p><div><txp:search_result_excerpt /></div>

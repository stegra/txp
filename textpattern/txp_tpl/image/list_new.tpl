			...
			<td colspan="99">
				...
				<div class="left">
					...
					<div class="view-mode">
						...
						<a class="grid" title="Grid View" href="index.php?event=image&view_mode=grid">Grid</a> | 
						<a class="list" title="List View" href="index.php?event=image&view_mode=list">List</a>
					</div>
...
<div class="prev-next" colspan="99">
	{if $images}<p>{$prevnext}</p>{/if}
</div>

{if $view_mode_list}

	<div class="show-column" colspan="99">
		<form action="index.php" method="post">
			
			<input type="hidden" name="event" value="image" />
			<input type="hidden" name="step" value="image_list_column" />
			
			Show column for: 
			<input type="checkbox" name="show_copyright" onclick="submit(this.form)" {if $show_copyright}checked="checked"{/if}>Copyright 
			<input type="checkbox" name="show_caption" onclick="submit(this.form)" {if $show_caption}checked="checked"{/if}>Caption 
			<input type="checkbox" name="show_alt" onclick="submit(this.form)" {if $show_alt}checked="checked"{/if}>Alt text 
			<input type="checkbox" name="show_keywords" onclick="submit(this.form)" {if $show_keywords}checked="checked"{/if}>Keywords 
...
{/if}
	...
	<div class="view-per-page" colspan="99">

<form action="index.php" data-mode="{$mode}" data-sort="{$sortby}" data-sortdir="{$sortdir}" action="" method="post" id="longform" name="longform" onsubmit="return verify('{$trash_cnt}')">
	...
	<table cellpadding="2" cellspacing="0" border="1" id="list" align="center">
	...
	<tr class="sort-{$sortby|lower}">
		...
		<th colspan="{$maxlevel}" class="title"><a title="{$linkdir_title}" href="index.php?sort=Title&dir={$linkdir}&event=list">Title</a></th>
	...
	<tr class="main-article">
		...
		{capture name=colspan}
			{if $col_image and !$image_name}{$maxlevel + 1}{else}{$maxlevel}{/if}
		{/capture}

		<td class="title" colspan="{$smarty.capture.colspan}">
	...
	{if $list_items_count lt 11}
		{section name=empty start=$list_items_count+1 loop=12}
			<tr class="even empty-{$smarty.section.empty.index}"><td colspan="99">&nbsp;</td></tr>
		{/section}
	{/if}
	...
	<tr class="footer">
		...
		<td colspan="99" align="right" valign="top" class="select">
	...
	<tr class="spacer">
		{section name=spacer start=2 loop=$maxlevel+2}
			<td width="10" class="level-{$smarty.section.spacer.index}"><img src="/textpattern/txp_img/spacer.gif" width="10" height="1"></td>
		{/section}
	</tr>

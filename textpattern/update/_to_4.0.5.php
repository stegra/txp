...
/*
$HeadURL: https://textpattern.googlecode.com/svn/releases/4.2.0/source/textpattern/update/_to_4.0.5.php $
...
*/
	...
	if ($update_files) {
		// $prefs = get_prefs();
		$file_base_path = fetch('val','txp_prefs','name','file_base_path');
		$rs = safe_rows('*', 'txp_file', '1=1');
		foreach ($rs as $row) {
			$path = build_file_path(@file_base_path, @$row['filename']);

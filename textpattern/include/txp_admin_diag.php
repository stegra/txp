...
$HeadURL: https://textpattern.googlecode.com/svn/releases/4.2.0/source/textpattern/include/txp_diag.php $
...
// -----------------------------------------------------------------------------
	...
	$files = array(
		...
		'/../css.php',
		'/css.php',
		'/include/txp_admin_admin.php',
		'/include/txp_admin_diag.php',
		'/include/txp_admin_import.php',
		'/include/txp_admin_log.php',
		'/include/txp_admin_plugin.php',
		'/include/txp_admin_prefs.php',
		'/include/txp_auth.php',
		'/include/txp_content_article.php',
		'/include/txp_content_category.php',
		'/include/txp_content_discuss.php',
		'/include/txp_content_file.php',
		'/include/txp_content_image.php',
		'/include/txp_content_link.php',
		'/include/txp_content_list.php',
		'/include/txp_presentation_css.php',
		'/include/txp_presentation_form.php',
		'/include/txp_presentation_page.php',
...
// -----------------------------------------------------------------------------
	...
	{
		global $PFX, $prefs, $files, $txpcfg, $step, $theme;
		extract(get_prefs());
 ...
 /* if ($rows = safe_rows('name, version, code_md5, md5(code) as md5', 'txp_plugin', 'status > 0')) {
	...
	$out[] = hed(gTxt('preflight_check'),1).n;
	
	if ($fail) {
		foreach ($fail as $help => $message)
			$out[] = tag(nl2br($message).sp.popHelp($help),'div',' class="not-ok"').n.n;
	}
	else {
		$out[] = tag(gTxt('all_checks_passed'),'div',' class="ok"').n.n;
	}
	
	echo preg_replace("/\\t/","&#160;&#160;&#160;&#160;",implode('',$out));

	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	
	echo hed(gTxt('diagnostic_info'),1).n.n;
	echo '<div style="font-family: Monaco">'.n;
	...
	$out = array(
		...
		tr(td(gTxt('txp_version').cs).td(txp_version.' ('.($rev ? 'r'.$rev : 'unknown revision').')')),

		tr(td(gTxt('last_update').cs).td(gmstrftime($fmt_date, $dbupdatetime).'/'.gmstrftime($fmt_date, @filemtime(txpath.'/update/_update.php')))),
		
		tr(td(gTxt('document_root').cs).td(@$_SERVER['DOCUMENT_ROOT']. (($real_doc_root != @$_SERVER['DOCUMENT_ROOT']) ? ' ('.$real_doc_root.')' : '') )),

		tr(td('$path_to_site'.cs).td($path_to_site)),

		tr(td(gTxt('txp_path').cs).td(txpath)),

		tr(td(gTxt('permlink_mode').cs).td($permlink_mode)),

		(ini_get('open_basedir')) ? tr(td('open_basedir'.cs).td(ini_get('open_basedir'))) : '',

		(ini_get('upload_tmp_dir')) ? tr(td('upload_tmp_dir'.cs).td(ini_get('upload_tmp_dir'))) : '',

		tr(td(gTxt('tempdir').cs).td($tempdir)),

		tr(td(gTxt('web_domain').cs).td($siteurl)),

		tr(td(gTxt('php_version').cs).td(phpversion())),

		($is_register_globals) ? tr(td(gTxt('register_globals').cs).td($is_register_globals)) : '',

		tr(td(gTxt('gd_library').cs).td($gd)),

		tr(td(gTxt('server').' TZ'.cs).td((timezone::is_supported() ? date_default_timezone_get() : ((getenv('TZ')) ? getenv('TZ') : '-')))),
		tr(td(gTxt('server_time').cs).td(strftime('%Y-%m-%d %H:%M:%S'))),
		tr(td(strip_tags(gTxt('is_dst')).cs).td($is_dst)),
		tr(td(strip_tags(gTxt('auto_dst')).cs).td($auto_dst)),
		tr(td(strip_tags(gTxt('gmtoffset')).cs).td($timezone_key.sp."($gmtoffset)")),

		tr(td('MySQL'.cs).td(mysql_get_server_info())),

		tr(td(gTxt('locale').cs).td($locale)),

		(isset($_SERVER['SERVER_SOFTWARE'])) ? tr(td(gTxt('server').cs).td($_SERVER['SERVER_SOFTWARE'])) : '',

		(is_callable('apache_get_version')) ? tr(td(gTxt('apache_version').cs).td(apache_get_version())) : '',

		tr(td(gTxt('php_sapi_mode').cs).td(PHP_SAPI)),

		tr(td(gTxt('rfc2616_headers').cs).td(ini_get('cgi.rfc2616_headers'))),

		tr(td(gTxt('os_version').cs).td(php_uname('s').' '.php_uname('r'))),

		($active_plugins ? tr(td(gTxt('active_plugins').cs).td(join(', ', $active_plugins))) : ''),

		tr(td(gTxt('theme_name').cs).td($theme_name.sp.$theme_manifest['version'])),
